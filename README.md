...
- Login, eight-hour signed session containing only `user_id`, and POST logout.
...
- **Signed cookie instead of opaque server-side sessions.** It is tamper-evident and
  simple for the assessment, but not centrally revocable or encrypted. Production
  would use centrally managed sessions or company SSO.
